Hi, ChatGPT! I was using a template on GitHub to make a post that contains LaTeX, but that template is not rendering the same LaTeX as in GitHub's preview mode. Is it possible to override the template and use GitHub's markdown functionality instead?
...
## Adam
Adam, a portmanteau of <ins>ada</ins>ptive <ins>m</ins>oment estimation, is another method with an adaptive learning rate, especially popular in the deep learning community where it was the optimizer chosen to train OpenAI's ChatGPT. Adam can be loosely thought of as RMSProp with windowed momentum. So, like in RMSProp, we will use an exponential moving average of squared gradients given here by $\color{black}\mathbf{v}\_t\color{black} = \beta\_2 \mathbf{v}\_{t-1} + (1-\beta\_2)\mathbf{g}\_t \circ \mathbf{g}\_t$, but also incorporate an exponential moving average of momentum given here by $\color{black}\mathbf{m}\_t\color{black} = \beta\_1 \mathbf{m}\_{t-1} + (1-\beta\_1)\mathbf{g}\_t$. Such properties yield a method that is almost Adam but not quite it, sometimes referred to as Adam (biased). Adam (biased) has the update rule  
