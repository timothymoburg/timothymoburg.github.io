...
## Adam
Adam, a portmanteau of <ins>ada</ins>ptive <ins>m</ins>oment estimation, is another method with an adaptive learning rate, especially popular in the deep learning community where it was the optimizer chosen to train OpenAI's ChatGPT. Adam can be loosely thought of as RMSProp with windowed momentum. So, like in RMSProp, we will use an exponential moving average of squared gradients given here by $\color{black}\mathbf{v}_{t} = \beta_2 \mathbf{v}_{t-1} + (1-\beta_2)\mathbf{g}_{t} \circ \mathbf{g}_{t}$, but also incorporate an exponential moving average of momentum given here by $\color{black}\mathbf{m}_{t}\color{black} = \beta_1 \mathbf{m}_{t-1} + (1-\beta_1)\mathbf{g}_{t}$.
