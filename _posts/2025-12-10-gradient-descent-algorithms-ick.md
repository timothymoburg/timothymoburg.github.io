...
In all of these methods, we will denote $\mathbf{\theta}\_{t}$ to be the parameters of our objective function at time $t$, we will denote $\mathbf{g}\_t$ to be the Euclidean gradient of our objective function with respect to our parameters at time $t$, we will denote $\circ$ to be the Hadamard product, and allow for broadcasting.
